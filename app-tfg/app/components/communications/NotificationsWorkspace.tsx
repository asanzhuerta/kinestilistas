...
export default function NotificationsWorkspace({
	...
	const normalizedNotificationPage = Math.min(
		currentNotificationPage,
		totalNotificationPages,
	);
	const visibleNotifications = notifications.slice(
		(normalizedNotificationPage - 1) * NOTIFICATIONS_PER_PAGE,
		normalizedNotificationPage * NOTIFICATIONS_PER_PAGE,
	);
				...
				<section className="space-y-3 rounded-3xl border border-slate-200 bg-white/85 p-5 shadow-sm">
					<div className="flex flex-col gap-3 xl:flex-row xl:items-start xl:justify-between">
						...
						<div className="flex flex-wrap items-center gap-2">
							<PushNotificationOptIn variant="inline" />
							<button
								type="button"
								onClick={markAllRead}
								...
								<div className="flex flex-wrap items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-600">
									...
									<div className="flex gap-2">
										<button
											type="button"
											onClick={() =>
												setCurrentNotificationPage((currentPage) =>
													Math.max(1, currentPage - 1),
												)
											}
											disabled={normalizedNotificationPage === 1}
										...
										<button
											...
											onClick={() =>
												setCurrentNotificationPage((currentPage) =>
													Math.min(totalNotificationPages, currentPage + 1),
												)
											}
											disabled={
												normalizedNotificationPage === totalNotificationPages
											}
											className="rounded-xl border border-slate-200 bg-white px-3 py-2 text-xs font-semibold text-slate-700 transition hover:bg-slate-100 disabled:cursor-not-allowed disabled:opacity-50"
										>
											Siguiente
