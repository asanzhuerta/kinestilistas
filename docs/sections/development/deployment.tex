...
En la demostración publicada se ha utilizado la combinación \texttt{Netlify + Neon + Cloudinary}. En este escenario resulta conveniente distinguir entre dos usos de la base de datos: para el tiempo de ejecución web, \texttt{Netlify} utiliza la cadena pooled de \texttt{Neon} como \texttt{DATABASE\_URL}; para migraciones manuales, restauración de datos o scripts locales puede emplearse una conexión directa equivalente si el proveedor la ofrece. Esta separación evita penalizar la capa web y reduce incidencias en tareas operativas puntuales.
...
\begin{itemize}
    \item Un entorno capaz de ejecutar \texttt{Node.js 20} o superior; para la demo desplegada se ha fijado \texttt{Node.js 22}.
...
Si se quiere forzar una URL distinta para el calculo servidor de duraciones de ruta, puede añadirse ademas:
