...
\begin{itemize}
    \item prioridad del uso móvil para comerciales y clientes, con navegación lateral desplegable, identificacion visual del rol y vistas compactas;
...
La navegación real del sistema parte de una entrada común en \texttt{/login}. Tras autenticarse, cada usuario es redirigido a la raíz funcional de su rol, desde donde accede a sus módulos disponibles. La Figura~\ref{fig:NavigationModel} resume este comportamiento.
...
\begin{figure}[H]
    ...
    \begin{tabularx}{\textwidth}{|p{0.17\textwidth}|X|}
        ...
        \hline
        Acceso común & \texttt{/login} $\rightarrow$ redirección a \texttt{/admin}, \texttt{/commercials} o \texttt{/clients} según el rol autenticado. \\
        \hline
        Administrador & \texttt{/admin} $\rightarrow$ usuarios, solicitudes, clientes, auditoría, catálogo, pedidos globales, comunicaciones y \texttt{/admin/settings}. Desde estos espacios se accede a listados, detalle, formularios de alta, exportación de auditoría y configuración de canales de avisos. Las pantallas de diagnóstico técnico de \texttt{M7} no forman parte de la navegación ordinaria del administrador. \\
        \hline
...
\subsection{Mockups pendientes de la interfaz}

En esta versión se ha decidido no incluir capturas reales dentro del apartado de modelo de interfaz, porque su función no es la misma que la de un mockup. Las capturas reales sirven como evidencia de pruebas o de ejecución, mientras que en este punto interesa representar la estructura prevista de las pantallas sin depender de datos reales ni del estado visual exacto de una sesión local.

Para dejar este punto cerrado con una base más limpia, queda pendiente preparar una pequeña serie de mockups documentados. Con cinco piezas sería suficiente:

\begin{itemize}
    \item mockup de acceso y panel inicial por rol;
    \item mockup de panel de administrador con navegación principal, auditoría y ajustes;
    \item mockup de gestión comercial con visitas, ruta diaria y detalle de reparto;
    \item mockup de pedido y detalle de pedido, diferenciando cliente y comercial cuando proceda;
    \item mockup de comunicaciones y avisos, incluyendo promociones, formaciones y notificaciones.
\end{itemize}

Estos mockups pueden construirse a partir de la interfaz final, pero deberían presentarse como composición simplificada y no como captura literal. Las capturas reales, si se quieren conservar, encajan mejor en el capítulo de pruebas o en un anexo de evidencias.
