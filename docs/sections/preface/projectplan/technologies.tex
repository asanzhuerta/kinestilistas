...
El almacenamiento externo de imágenes se resuelve mediante \textbf{Cloudinary}, tanto para la imagen de perfil de usuario como para las imágenes del catálogo administrado por el distribuidor \citep{cloudinaryupload2026}. Para la representación del QR operativo de cada reparto comercial se utiliza \textbf{QuickChart}, que permite generar imágenes QR a partir de una carga textual estable \citep{quickchartqr2026}.

Esta decisión evita introducir una dependencia adicional de renderizado de QR en el frontend y facilita que la misma referencia pueda reutilizarse para visualización, impresión y validación en reparto. Cuando el pedido se gestiona por agencia, la etiqueta mantiene el mismo formato imprimible, pero sustituye el bloque QR por una marca textual para distinguir la preparación logística.
...
\begin{itemize}
    ...
    \item una estrategia sencilla de cache de \textit{app shell}, iconos, manifiesto y pantalla offline segura cuando la red falla.
\end{itemize}
...
El \textit{service worker} evita cachear rutas de API y zonas privadas como pedidos, rutas, perfiles o administración. Ante una caída de red, las navegaciones protegidas se degradan hacia \path{/offline}, manteniendo una experiencia informativa sin presentar datos operativos posiblemente obsoletos.

\textcolor{red}{No se trata todavía de una PWA avanzada con sincronización offline completa}, pero sí de una base coherente con el uso móvil previsto para la aplicación.
...
En conjunto, la pila tecnológica elegida permite sostener una aplicación web moderna, modular y evolutiva, manteniendo una relación directa entre requisitos, implementación y despliegue.
